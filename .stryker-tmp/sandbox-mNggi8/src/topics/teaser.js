...
module.exports = function (Topics) {
	Topics.getTeasers = async function (topics, options) {
		...
		const [allPostData, callerSettings] = await Promise.all([
			posts.getPostsFields(teaserPids, ['pid', 'uid', 'timestamp', 'tid', 'content', 'sourceContent', 'answered']),
	...
	async function getPreviousNonBlockedPost(postData, blockedUids) {
		...
		do {
			...
			const prevPosts = await posts.getPostsFields(pids, ['pid', 'uid', 'timestamp', 'tid', 'content', 'answered']);
